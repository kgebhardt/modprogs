...
C=============================================================================D
      ...
      print *,rmin,cden,core
      call density()
      call haloread()
      fac1 = sqrt(GG*totlight*arcsec/distance/angrad)

      write (6,100)
100   format ('Do you have a tables.out file you want to use?  y/n  ',$)
      read (5,*)dummy
...
9     format ('      Nskip = ',$)
 ...
 676  write(*,"('# crossings, Npdat, epsilon fac : '$)")
      read(*,*) Nsos2,Npdat2,efac
...
C---- END TIME LOOP
      ...
      print *
      print *,'ivmin, ivmax ',ivminp,ivmaxp
...
c      tt = t*2.*float(Npdat2)
      ...
      print*
      print*
      print*,'internal moments: sr,st,sp'
      print*

      open(unit=14,file='oneorbit.moments.out',status='unknown')
      write(*,'(" ir  iv   r[arcsec]   sr    st    sp    #iter",
     &     "     v0liba")')
     ...
     &     "     v0liba")')
      do ira=1,Nrani
         ...
         if(idot(ira,ita).ne.0) then
            write(14,'(2(i3,1x),3x,f6.2,3x,3(f5.1,1x),2x,i5,3x,f8.5,
     &           2x,f8.5)') 
     &           ira,ita,
     &           RneeIRC(ira)*angrad,xsr,xst,xsp,idot(ira,ita),
     &           v0liba(ira,ita),vr2liba(ira,ita)
            write(*,'(2(i3,1x),3x,f6.2,3x,3(f5.1,1x),2x,i5,3x,f8.5,
     &           2x,f8.5)') 
     &           ira,ita,
     &           RneeIRC(ira)*angrad,xsr,xst,xsp,idot(ira,ita),
     &           v0liba(ira,ita),vr2liba(ira,ita)
     ...
     &              RneeIRC(ira)*angrad,dEm(ira,ita)
               write(*,'(2(i3,1x),3x,f6.2,3x,(f11.8))') 
     &              ira,ita,RneeIRC(ira)*angrad,dEm(ira,ita)
            end if
         end do
      end do
      close(14)
      ...
      write(*,'(" ir  iv   r[arcsec]    rm/dr  rm/dv",    
     ...
     &              dr(ira,ita)/dv(ira,ita),ivisit(ira,ita),ipt
               write(*,'(2(i3,1x),3x,f6.2,5x,2(f6.3,1x),1x,
     &              f5.2,2x,i5,6x,i5)')
     &              ira,ita,
     &              RneeIRC(ira)*angrad,xrm/dr(ira,ita),xrm/dv(ira,ita),
     &              dr(ira,ita)/dv(ira,ita),ivisit(ira,ita),ipt
...
c      write (6,*)' '
      ...
      esig=sqrt((sum1+float(nsum)*eavg*eavg-2.*eavg*sum2)/float(nsum))
c      print*,nsum,sum2,eavg,sum1
      write(6,*) ''
      print *,'Average E, Sig, Sig/E = ',eavg,esig,esig/eavg
