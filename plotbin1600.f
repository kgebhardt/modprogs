...
c         read(1,*,end=668) xp,yp,vel,dvel
c         write(cvel,2001) vel,dvel
         x=xp*ca-yp*sa
         y=xp*sa+yp*ca
c         print *,x,y
         ivel=1
         if(x.ge.0.and.y.ge.0) iq=1
         if(x.lt.0.and.y.ge.0) iq=2
         if(x.lt.0.and.y.lt.0) iq=3
         if(x.ge.0.and.y.lt.0) iq=4
         call pgsci(1)
         if(x.lt.0) then
            x=-x
            ivel=0
...
c            ivel=1
         ...
         call writebin(ir,iv,cvel,fname,i,iq)
 ...
 1201 format(a7,2(1x,f8.3))
      ...
      subroutine writebin(ir,iv,cvel,fname,i,iq)
      character fname*20,cvel*60
      if(ir.lt.1.or.iv.lt.1) then
         print *,'Not in Array',ir,iv,i,cvel
         return
      endif
      if(iq.eq.1) fname(1:3)='qb1'
      if(iq.eq.2) fname(1:3)='qb2'
      if(iq.eq.3) fname(1:3)='qb3'
      if(iq.eq.4) fname(1:3)='qb4'
