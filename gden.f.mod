C This program calculates the mass in each bin for the library
...
c      aelz=rtbiso(funcelz,.0001,.9,1.e-5)
 ...
 2400 continue
      ...
      do i=1,nmax
         mr=mr+1
         na=0
         read(1,*,end=666) x1,x3,x4
         do j=1,step
            na=na+1
c            read(1,*,end=666) x1,x2,x3,x4
c            td(na)=x2            !theta (rad)
            tin(na)=td(na)
...
c            call halodens(x1,x2,xaddmass)
            xaddmass=0.
